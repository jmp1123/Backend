 ...
 *   3. node server.js
 *
 * Deploy options: Railway, Render, Fly.io, Heroku, VPS — all work.
 */
...
// CORS — in production, replace '*' with your actual front-end domain
// e.g. 'https://vestiary.app' or wherever you're hosting the HTML file
app.use(cors({
...
// 30 requests per user per minute is generous for normal use
...
// ── POST /api/identify ───────────────────────────────────────────────────────
// Accepts a base64 image, sends it to Claude, returns clothing metadata as JSON.
//
// Request body:
//   { image: "<base64 string>", mimeType: "image/jpeg" }
//
// Response (success):
//   { name, cat, color, fabric, fit, brand, details }
//
// Response (error):
//   { error: "<message>" }
...
app.post('/api/identify', async function (req, res) {
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model:      'claude-haiku-4-5-20251001', // fast + cheap for image ID
        max_tokens: 300,
        messages: [{
          role: 'user',
          content: [
    ...
    // Extract JSON even if Claude added a stray leading sentence
    ...
    // Sanitise — make sure all expected fields are present
    const out = {
...
// ── POST /api/coach ─────────────────────────────────────────────────────────
// Style coach chat — keeps API key off the client.
//
// Request body:
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 400,
        system:     systemPrompt,
        messages:   messages.slice(-10) // last 10 turns to keep costs low
      })
    });
