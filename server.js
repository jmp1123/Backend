...
// Trust proxy for Railway/Heroku (fixes rate limiter warning)
...
// ============== BACKGROUND REMOVAL ==============
// Advanced background removal using flood fill + edge detection
app.post('/api/remove-bg', async (req, res) => {
  ...
  try {
    ...
    // Resize for processing (larger = better quality but slower)
    const maxSize = 1024;
    let processWidth = metadata.width;
    let processHeight = metadata.height;
    if (processWidth > maxSize || processHeight > maxSize) {
      if (processWidth > processHeight) {
        processHeight = Math.round(processHeight * maxSize / processWidth);
        processWidth = maxSize;
      } else {
        processWidth = Math.round(processWidth * maxSize / processHeight);
    ...
    // Sample edges to find background color (more samples = better detection)
    const edgeSamples = [];
    ...
    for (let x = 0; x < width; x += Math.max(1, Math.floor(width / 20))) {
      ...
      if (top) edgeSamples.push(top);
      if (bottom) edgeSamples.push(bottom);
    }
    ...
    for (let y = 0; y < height; y += Math.max(1, Math.floor(height / 20))) {
      ...
      if (left) edgeSamples.push(left);
      if (right) edgeSamples.push(right);
    }
    
    // Find most common color among edge samples (background color)
    const colorCounts = {};
    edgeSamples.forEach(p => {
    ...
    // Get the most common edge color
    ...
    // Color distance function (Euclidean in LAB-like space for better perceptual matching)
    const colorDistance = (p, bg) => {
      // Weight green channel more (human eye is more sensitive to green)
      const rDiff = (p.r - bg.r) * 0.3;
      const gDiff = (p.g - bg.g) * 0.59;
      const bDiff = (p.b - bg.b) * 0.11;
      return Math.sqrt(rDiff * rDiff + gDiff * gDiff + bDiff * bDiff) * 2;
    };
    
    // Check if a color is likely "white-ish" or "gray-ish" background
    ...
    // Dynamic tolerance based on background color
    const bgBrightness = (bgColor.r + bgColor.g + bgColor.b) / 3;
    const isNeutralBg = isNeutralColor(bgColor.r, bgColor.g, bgColor.b);
    
    // More aggressive tolerance for white/neutral backgrounds
    let baseTolerance;
    if (bgBrightness > 230 && isNeutralBg) {
      baseTolerance = 45; // Very white background - be aggressive
    } else if (bgBrightness > 200 && isNeutralBg) {
      baseTolerance = 40;
    } else if (bgBrightness > 150) {
      baseTolerance = 50;
    } else {
      baseTolerance = 60;
    }
    
    console.log('Background brightness:', bgBrightness, 'Tolerance:', baseTolerance, 'Neutral:', isNeutralBg);
    ...
    // Add all edge pixels to queue (sample more densely)
    for (let x = 0; x < width; x++) {
      queue.push({ x, y: 0 });
      queue.push({ x, y: 1 });
      queue.push({ x, y: height - 1 });
      queue.push({ x, y: height - 2 });
    }
    for (let y = 0; y < height; y++) {
      queue.push({ x: 0, y });
      queue.push({ x: 1, y });
      queue.push({ x: width - 1, y });
      queue.push({ x: width - 2, y });
    }
    ...
    while (queue.length > 0) {
      ...
      // Also check if pixel is very bright/white (common background even if not exact match)
      const pixelBrightness = (pixel.r + pixel.g + pixel.b) / 3;
      const isLikelyBg = dist < baseTolerance || 
        (pixelBrightness > 240 && isNeutralColor(pixel.r, pixel.g, pixel.b));
      
      // If pixel is similar to background, make it transparent
      if (isLikelyBg) {
        setTransparent(pixel.idx);
        
        // Add neighbors to queue (8-connected for better coverage)
    ...
    // Second pass: clean up isolated background pixels inside the subject
    // and smooth edges with anti-aliasing
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        ...
        const pixel = { r: newData[idx], g: newData[idx + 1], b: newData[idx + 2] };
        const dist = colorDistance(pixel, bgColor);
        ...
        neighbors.forEach(nIdx => {
          if (newData[nIdx * 4 + 3] === 0) transparentNeighbors++;
        });
        
        // If surrounded by many transparent pixels and similar to bg, make semi-transparent
        if (transparentNeighbors >= 2 && dist < baseTolerance * 1.5) {
          // Anti-alias: partial transparency based on distance
          const alpha = Math.min(255, Math.max(0, Math.round((dist / baseTolerance) * 255)));
          newData[idx + 3] = alpha;
        }
      }
    }
    ...
    console.log('Background removal complete, processed', processed.size, 'pixels');
